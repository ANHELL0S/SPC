...
export class CategoryModel {
	...
	static async create({ data }) {
		try {
			const newCategory = await categoryScheme.create(data)
			return newCategory
		} catch (error) {
...
categoryScheme.afterSync(async () => {
	try {
		const existingData = await categoryScheme.findOne()
		if (!existingData) {
			const data_default = [{ name: 'Revista' }, { name: 'Libros' }, { name: 'Memorias' }]

			// Iterar sobre los datos y crear cada registro
			for (const data of data_default) {
				await categoryScheme.create(data)
			}
		}
	} catch (error) {
