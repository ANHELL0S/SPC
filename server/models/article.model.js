...
export class ArticleModel {
	...
	static async create(newData) {
		try {
			const newArticle = await articleScheme.create(newData)
			return newArticle
		} catch (error) {
	...
	static async delete(id) {
		try {
			...
			// Eliminar registros relacionados en line research
			await relation_line_research_article_Scheme.destroy({
				where: { id_article_fk: id },
			})

			// Eliminar registros relacionados dependence
			await relation_field_dependence_article_Scheme.destroy({
				where: { id_article_fk: id },
			})

			// Eliminar registros relacionados detailed
			await relation_field_details_article_Scheme.destroy({
				where: { id_article_fk: id },
			})

			// Eliminar registros relacionados specific
			await relation_field_specific_article_Scheme.destroy({
				where: { id_article_fk: id },
			})

			// Eliminar registros relacionados wide
			await relation_field_wide_article_Scheme.destroy({
				where: { id_article_fk: id },
			})

			// Eliminar registros relacionados carrer
			await relation_career_article_Scheme.destroy({
				where: { id_article_fk: id },
			})
