...
import { ModalCreate, ModalUpdate, ModalRemove } from '../../components/form/FormsCareer'

const CareerLayout = () => {
	const [search, setSearch] = useState('')
	const { career, loading, error, setCareer } = useAllCareer()
	...
	const filteredData = filterData(career, search, ['name'])
			...
			{loading ? (
				...
				<>
					<main className='container mx-auto px-4 lg:px-8 xl:max-w-7xl mt-20'>
						<div className='flex flex-col gap-4 gap-x-20'>
							<div className='flex items-center justify-between'>
								<div className='flex gap-4 text-md font-semibold text-neutral-500'>
									<span>Carreras ({career.length})</span>
								</div>
							</div>
						...
						{showCreateModal && (
							<ModalCreate career={selected} onClose={toggleCreateModal} onCreate={handleCreateCallback} />
						)}

						{showUpdateModal && (
							<ModalUpdate career={selected} onClose={toggleUpdateModal} onUpdate={handleUpdateCallback} />
						)}

						{showDeleteModal && (
							<ModalRemove career={selected} onClose={toggleDeleteModal} onDelete={handleDeleteCallback} />
						)}
					</main>
				</>
			)}
