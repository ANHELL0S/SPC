...
import { ModalCreate, ModalUpdate, ModalRemove } from '../../components/form/FormsFaculty'

const FacultyLayout = () => {
	const [search, setSearch] = useState('')
	const { faculties, loading, error, setFaculty } = useAllFaculty()
	...
	const filteredData = filterData(faculties, search, ['name'])
	...
	const columns = [
		...
		},
		{
			...
			sortable: true,
			minWidth: '0px',
			cell: row => <div className='text-xs text-neutral-500'>{row.usersCount}</div>,
		},
		{
			name: 'N artículos',
			...
			{loading ? (
				...
				<>
					<main className='container mx-auto px-4 lg:px-8 xl:max-w-7xl mt-20'>
						<div className='flex flex-col gap-4 gap-x-20'>
							<div className='flex items-center justify-between'>
								<div className='flex gap-4 text-md font-semibold text-neutral-500'>
									<span>Facultades ({faculties.length})</span>
								</div>
							</div>
						...
						{showCreateModal && (
							<ModalCreate faculty={selected} onClose={toggleCreateModal} onCreate={handleCreateCallback} />
						)}

						{showUpdateModal && (
							<ModalUpdate faculty={selected} onClose={toggleUpdateModal} onUpdate={handleUpdateCallback} />
						)}

						{showDeleteModal && (
							<ModalRemove faculty={selected} onClose={toggleDeleteModal} onDelete={handleDeleteCallback} />
						)}
					</main>
				</>
			)}
