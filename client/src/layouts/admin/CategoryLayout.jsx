...
import { useAllCategory } from '../../hooks/useCategories'
...
const CategoryLayout = () => {
	...
	const { category, loading, error, setCategory } = useAllCategory()
	...
	const filteredData = filterData(category, search, ['name'])
	...
	const columns = [
		...
		},
		{
			...
			sortable: true,
			minWidth: '150px',
			cell: row => <div className='text-xs text-neutral-500'>{row.articleCount}</div>,
		},
		{
			name: 'Estado',
			...
			{loading ? (
				...
				<>
					<main className='container mx-auto px-4 lg:px-8 xl:max-w-7xl mt-20'>
						<div className='flex flex-col gap-4 gap-x-20'>
							<div className='flex items-center justify-between'>
								<div className='flex gap-4 text-md font-semibold text-neutral-500'>
									<span>Categorias ({category.length})</span>
								</div>
							</div>
						...
						{showActiveModal && (
							<ModalReactive category={selected} onClose={toggleActiveModal} onActive={handleActiveCallback} />
						)}

						{showDeleteModal && (
							<ModalRemove category={selected} onClose={toggleDeleteModal} onRemove={handleDeleteCallback} />
						)}
					</main>
				</>
			)}
