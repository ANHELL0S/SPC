...
const TagsLayout = () => {
	...
	const { tag, loading, error, setTags } = useAllTags()
	...
	const filteredData = filterData(tag, search, ['name'])
			...
			{loading ? (
				...
				<>
					<main className='container mx-auto px-4 lg:px-8 xl:max-w-7xl mt-20'>
						<div className='flex flex-col gap-4 gap-x-20'>
							<div className='flex items-center justify-between'>
								<div className='flex gap-4 text-md font-semibold text-neutral-500'>
									<span>Etiquetas ({tag.length})</span>
								</div>
							</div>
						...
						{showActiveModal && (
							<ModalReactive tag={selected} onClose={toggleActiveModal} onActive={handleActiveCallback} />
						)}

						{showDeleteModal && (
							<ModalRemove tag={selected} onClose={toggleDeleteModal} onRemove={handleDeleteCallback} />
						)}
					</main>
				</>
			)}
