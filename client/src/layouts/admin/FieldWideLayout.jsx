...
import { ModalCreate, ModalUpdate, ModalRemove } from '../../components/form/FormsFieldWide'

const FieldWideLayout = () => {
	const [search, setSearch] = useState('')
	const { field_wide, loading, error, setField_wide } = useAllField_wide()
	...
	// Update
	const handleUpdate = field_wide => {
		setSelected(field_wide)
	...
	const filteredData = filterData(field_wide, search, ['name'])
			...
			{loading ? (
				...
				<>
					<main className='container mx-auto px-4 lg:px-8 xl:max-w-7xl mt-20'>
						<div className='flex flex-col gap-4 gap-x-20'>
							<div className='flex items-center justify-between'>
								<div className='flex gap-4 text-md font-semibold text-neutral-500'>
									<span>Campos amplios ({field_wide.length})</span>
								</div>
							</div>
						...
						{showCreateModal && (
							<ModalCreate field_wide={selected} onClose={toggleCreateModal} onCreate={handleCreateCallback} />
						)}

						{showUpdateModal && (
							<ModalUpdate field_wide={selected} onClose={toggleUpdateModal} onUpdate={handleUpdateCallback} />
						)}

						{showDeleteModal && (
							<ModalRemove field_wide={selected} onClose={toggleDeleteModal} onDelete={handleDeleteCallback} />
						)}
					</main>
				</>
			)}
